    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void AimShoot()
        {
            ...
            if (_input.isAiming && Grounded && !_input.sprint)
            {
                ...
                Vector3 aimDirection = (transform.position - playerFollowCamera.transform.position).normalized;
                aimDirection.y = 0f; // Ensure we only rotate around the horizontal plane

                // Rotate the player to face opposite of the camera's direction
        ...
        public void Shoot()
        {
            ...
            arrow.transform.Rotate(0f, 90f, 90f);

            // Add a forward force to shoot the arrow
            Rigidbody rb = arrow.GetComponent<Rigidbody>();
            if (rb != null)
            {
                rb.isKinematic = false;  // Ensure the Rigidbody is not kinematic before shooting
